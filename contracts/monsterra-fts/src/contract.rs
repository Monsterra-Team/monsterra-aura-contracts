...
use crate::msg::{ExecuteMsg, InstantiateMsg, MigrateMsg, QueryMsg, TransferLogResponse};
use crate::state::{
    MinterData, TokenInfo, TransferLog, ALLOWANCES, ALLOWANCES_SPENDER, BALANCES, LOGO,
    LOG_COUNTER, MARKETING_INFO, TOKEN_INFO, TRANSFER_LOGS,
};
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::AllTransferLogs { start_after, limit } => {
            to_binary(&query_transfer_logs(deps, start_after, limit)?)
        }
    }
}

pub fn query_transfer_logs(
    deps: Deps,
